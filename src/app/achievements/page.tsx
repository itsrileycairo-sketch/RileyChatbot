...
// 🔥 FIX: Path yang bener tanpa folder /app/
...
export default function Achievements() {
  ...
  if (!data) return <CyberpunkLoading text="Decrypting Credentials..." />;
          ...
          achList.map((ach: any, index: number) => (
            <div key={ach.id} style={{ animationDelay: `${index * 150}ms` }} className="bg-white/50 dark:bg-[#0c0c1d]/80 backdrop-blur-xl border border-white/50 dark:border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center hover:border-cyan-500/50 transition-all duration-500 group shadow-lg dark:shadow-xl hover:shadow-[0_0_30px_rgba(6,182,212,0.2)] hover:-translate-y-1 animate-fade-in-up">
              ...
              <div className="w-full md:w-3/4">
                ...
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2 transition-colors group-hover:text-cyan-500">{ach.posisi || ach.judul}</h3>
                <h4 className="text-lg text-cyan-600 dark:text-cyan-500 mb-3 transition-colors">{ach.perusahaan || ach.instansi}</h4>
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm transition-colors font-light">{ach.deskripsi}</p>
              </div>
            </div>
          ))
        )}
